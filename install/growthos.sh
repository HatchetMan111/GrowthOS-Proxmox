...
# https://github.com/<GH_USER>/<GH_REPO> (Platzhalter, siehe README)
#
...
#
# Einzeiler:
#   bash -c "$(wget -qLO - https://raw.githubusercontent.com/<GH_USER>/<GH_REPO>/main/install/growthos.sh)"
#
...
# ---------------------------------------------------------------------------
APP="growthos"
GH_REPO="${GH_REPO:-https://github.com/CHANGE_ME/growthos.git}"
...
on_error() {
  ...
  msg_warn "Für eine vollständig ausführliche Fehlersuche erneut ausführen mit:"
  echo "    bash -x $0 2>&1 | tee ${LOG_FILE}.debug"
  exit "${exit_code}"
}
...
preflight() {
  ...
  if [[ "${GH_REPO}" == *"CHANGE_ME"* ]]; then
    ...
    msg_error "  GH_REPO=https://github.com/DEIN-USER/growthos.git bash -c \"\$(wget -qLO - <URL>)\""
    exit 1
  fi
...
install_app() {
  ...
  if [[ "${UPDATE_MODE}" -eq 1 ]] && pct exec "${CTID}" -- test -d "/opt/${APP}/.git" &>/dev/null; then
    ...
    pct exec "${CTID}" -- bash -c "cd /opt/${APP} && git fetch --quiet && git checkout ${GH_BRANCH} --quiet && git pull --quiet"
  else
    msg_info "Klone ${GH_REPO} (Branch ${GH_BRANCH})..."
    pct exec "${CTID}" -- bash -c "rm -rf /opt/${APP}_tmp && git clone --quiet --branch ${GH_BRANCH} ${GH_REPO} /opt/${APP}_tmp"
    pct exec "${CTID}" -- bash -c "
      mkdir -p /opt/${APP}
      shopt -s dotglob
      cp -r /opt/${APP}_tmp/* /opt/${APP}/
      rm -rf /opt/${APP}_tmp
    "
  fi
  ...
  if command -v pct &>/dev/null && pct exec "${CTID}" -- command -v ufw &>/dev/null; then
