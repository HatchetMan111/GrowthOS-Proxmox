...
async def dashboard(request: Request):
    with get_conn() as conn:
        ...
        recent_plans = conn.execute(
            "SELECT * FROM plans ORDER BY id DESC LIMIT 5"
        ).fetchall()
...
async def settings_save(
    ...
    set_setting("base_url", base_url.strip())
    if api_key.strip():
        set_setting("api_key", api_key.strip())
    set_setting("daily_token_cap", str(int(daily_token_cap or 0)))
    set_setting("max_tokens_per_request", str(int(max_tokens_per_request or 2000)))
